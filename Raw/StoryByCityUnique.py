...
FileName = Connect.FilePath + "StoryByCityUnique.txt"
    ...
    for post in posts['data']:
        for entry in post['values']:
            for key, value in entry['value'].items():
                Functions.WriteFile('StoryByCityUnique',str(item),post['name'],post['period'],post['title'],post['description'],post['id'],key,value,entry['end_time'])
